...
function defaultBrowserProjectId(): string {
  ...
  if (!connected.length) {
    throw new ExternalEditorCallError(
      'rejected',
      'No Vidit editor is connected. Call target_project with an existing project id for offline editing, or open the editor.',
    );
  }
  throw new ExternalEditorCallError(
    'rejected',
    'Multiple Vidit projects are open; call target_project with the intended project.',
  );
}
...
export function bindBrowserForCall(
  ...
  if (!binding || !editorBindingMatches(binding)) {
    throw new ExternalEditorCallError('rejected', `Project ${projectId} is not open in a connected Vidit editor.`);
  }
