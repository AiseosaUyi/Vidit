...
/**
 * Bridge Vidit's tool catalog into SDK tools. The handler emits
...
function hostTools(request: CopilotTurnRequest, state: () => TurnSession | undefined): Tool<any>[] {
  return request.tools.map((spec) => defineTool(spec.name, {
    ...
    // Vidit runs its own approval gate (approval-mode.ts); the CLI must
    ...
    handler: async (args: unknown) => {
      ...
      if (!outcome.success) {
        ...
        // strip Vidit's diagnostic, which the agent needs in order to
...
export function copilotSessionConfig(
  ...
  return {
    ...
    onPermissionRequest: () => ({
      kind: 'reject',
      feedback: 'Vidit only permits its own editing tools.',
    }),
