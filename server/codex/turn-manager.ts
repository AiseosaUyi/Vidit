...
function browserFailureSummary(result: unknown): string {
  ...
  return 'Vidit tool execution failed.';
}
...
function baseInstructions(request: CodexTurnRequest): string {
  const mode = request.askOnly
    ? [
        ...
        'You may call the provided read-only Vidit tools when current skill or project evidence is needed.',
      ].join(' ')
    : [
        'Use only the provided Vidit editor tools when the task needs project reads or changes.',
        'Their results come from the existing Vidit proposal workflow; never claim a change succeeded before its tool result confirms success.',
      ].join(' ');
  return [
    request.system.trim(),
    'Vidit runtime rules:',
...
export class CodexTurnManager {
  ...
  private serverRequest(session: TurnSession, request: CodexServerRequest): boolean {
    ...
    if (!callId || !name || !session.toolNames.has(name) || session.pendingTools.has(callId)) {
      const message = 'This Vidit tool call is unavailable. It was not part of this request (stale tool list, duplicate call, or malformed id). Tell the user to open the project and retry; if it persists, start a new run.'
  ...
  private abortPendingTools(session: TurnSession): void {
    for (const pending of session.pendingTools.values()) {
      pending.request.respond({
        contentItems: [{ type: 'inputText', text: 'Vidit cancelled this tool call.' }],
        success: false,
      });
    }
