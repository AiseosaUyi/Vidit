...
export const EXTERNAL_SESSION_TOOLS: readonly ExternalRegisteredTool[] = [
  ...
  },
  {
    ...
    description: 'Start an isolated Vidit edit draft. Manual mode waits for proposal review; auto mode applies only the staged proposal at review_edit_session. Real-project tools always require separate confirmation.',
    input_schema: {
      type: 'object',
      properties: {
        ...
        approvalMode: {
          type: 'string',
          enum: ['manual', 'auto'],
          description: 'manual (default) requires proposal approval in Vidit; auto applies the reviewed draft only and never bypasses real-tool confirmation.',
        },
      },
    },
  ...
  },
  {
    ...
    description: 'Cancel a draft or pending review without changing the live Vidit project.',
    input_schema: {
      type: 'object',
      properties: { editSessionId: SESSION_ID_PROPERTY },
      required: ['editSessionId'],
...
export function externalDraftSchemas(tools: readonly AgentToolSchema[]): ExternalRegisteredTool[] {
  return tools.map((tool) => {
    ...
    const scopeDescription = multiplexesPersistentActions
      ? 'Read and ProjectDoc actions use the edit-session draft; owned-library writes act on live local data and require one-shot Vidit confirmation. Pass editSessionId.'
...
export function externalRealSchemas(tools: readonly AgentToolSchema[]): ExternalRegisteredTool[] {
  return tools.filter((tool) => isExternalRealTool(tool.name)).map((tool) => ({
    ...withSession(
      tool,
      `${tool.description ?? tool.name} Acts on the live project. Every invocation needs a one-shot Vidit confirmation bound to this session/run, tool, and exact argument digest; changed arguments require a new confirmation. Pass editSessionId.`,
    ),
