...
<body><p style="font: 400 32px 'Noto Sans SC'">离线字体渲染 Vidit 123</p></body></html>`;
...
try {
  ...
  const evaluateSource = String.raw`(async () => {
    ...
    for (const weight of [100, 400, 900]) {
      ...
      context.fillText('离线字体渲染 Vidit 123', 8, 72);
    ...
    return {
      ...
      checks: [100, 400, 900].map((weight) =>
        document.fonts.check(weight + ' 32px "Noto Sans SC"', '离线字体渲染 Vidit 123')),
      rasters,
    };
  })()`;
