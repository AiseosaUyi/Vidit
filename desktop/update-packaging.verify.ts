...
for (const blockmap of [
  ...
  assert.ok(
    workflow.includes(`release-files/Vidit-\${EXPECTED_VERSION}-${blockmap}`),
...
assert.doesNotMatch(
  workflow,
  /find release -type d -name Vidit\.app|(?:mac|win|linux)-unpacked\b/,
...
assert.match(
  workflow,
  /"\$mounted_app\/Contents\/MacOS\/Vidit"/,
...
assert.match(
  workflow,
  /Vidit\.app\/Contents\/MacOS\/Vidit/,
  'macOS update ZIP must contain the application executable',
);
assert.match(
  workflow,
  /Vidit\.app\/Contents\/Frameworks\/Electron Framework\.framework\/Versions\/A\/Electron Framework/,
  'macOS update ZIP must contain the Electron runtime',
);
assert.match(
  workflow,
  /render_runtime="Vidit\.app\/Contents\/Resources\/chrome-headless-shell\//,
